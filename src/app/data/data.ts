import {
  faTwitter,
  faInstagram,
  faLinkedin,
  faTelegram,
} from "@fortawesome/free-brands-svg-icons";

export const socialLinks = [
  {
    name: "Twitter",
    link: "https://twitter.com/MikeMatovu_",
    icon: faTwitter,
  },

  {
    name: "Instagram",
    link: "https://www.instagram.com/mikematovu21/",
    icon: faInstagram,
  },
  {
    name: "Linkedin",
    link: "https://www.linkedin.com/in/matovu-mike-462654222/",
    icon: faLinkedin,
  },
  {
    name: "Telegram",
    link: "https://t.me/mikem43",
    icon: faTelegram,
  },
];



export const talks = [
  {
    title: "Building cross platform apps with Compose Multiplatform",
    event: "I/O Extended Mbarara",
    date: "July 13, 2024",
    description:
      "At the I/O Extended mbarara, I gave an introduction to Compose Multiplatform. Learn how to build cross-platform UIs with ease using Compose.",
  },
  // Add more talks here in the future
];